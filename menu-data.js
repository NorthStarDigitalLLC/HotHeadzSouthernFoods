 ...
 */
var CLOUD_MENU_URL = 'https://script.google.com/macros/s/AKfycbxHd4heBpNJzAsgodKJMlgq8dVa_qlpHjIUHHRFYL6AIJys-d4x5Mxm1P_egFfvQiNtnQ/exec';
...
// Fetch lunch dates from Google Sheets via JSONP
(function() {
  ...
  window[cb] = function(result) {
    ...
    if (result && result.success && result.dates) {
      ...
      for (var i = 0; i < keys.length; i++) {
        MENU_DATA.menus[keys[i]] = dates[keys[i]];
      }
      console.log('[HH] Loaded ' + keys.length + ' lunch dates from cloud');
      window.dispatchEvent(new CustomEvent('menuDataReady', { detail: MENU_DATA }));
    } else {
      console.log('[HH] No lunch dates from cloud');
    }
  };
  var s = document.createElement('script');
  s.id = cb;
  s.src = CLOUD_MENU_URL + '?action=lunch&callback=' + cb;
  s.onerror = function() { delete window[cb]; s.remove(); console.warn('[HH] Cloud lunch fetch failed'); };
  document.head.appendChild(s);
  setTimeout(function() { if (window[cb]) { delete window[cb]; var el = document.getElementById(cb); if (el) el.remove(); console.warn('[HH] Cloud lunch timed out'); } }, 15000);
})();
